...
-- 1) Display the department number, total salary payout and total commission payout for each department
 ...
 -- 2) Display the department number and number of employees in each department.
 select department_id as department_number, count(employee_id) as number_of_employees from employees group by department_id; 

-- 3)Display the department number and total salary of employees in each department.
select department_id as department_number, sum(salary) as total_salary_of_employees from employees group by department_id; 

-- 4) Display the employee's name who doesn't earn a commission. Order the result set without using the column name
 select concat(first_name,' ',last_name) as employee_name from employees where commission_pct is null; 
 
 
 -- 5) Display the employees name, department id and commission. If an Employee doesn't earn the commission, then display as 'No commission'. Name the columns appropriately
 ...
 -- 6) Display the employee's name, salary and commission multiplied by 2. If an Employee doesn't earn the commission, then display as 'No commission. Name the columns appropriately
...
-- 7) Display the sum of salaries of the employees working under each Manager.
select manager_id, sum(salary) from employees where manager_id is not null group by manager_id;

-

-- 8) Display the average of sum of the salaries and group the result with the department id. Order the result with the department id.
select avg(salary),department_id as avg_of_salary from employees group by department_id order by department_id ;

-- 9) Select the maximum salary of each department along with the department id.
